...
class LWM2MClient extends EventEmitter {
  ...
  register(host, port, url, endpointName) {
    ...
    this.startListener(deviceInformation);
  }
   ...
   * @memberof LWM2MClient
   */
  createRegisterResponseHandler(deviceInformation, callback) {
   ...
   * @memberof LWM2MClient
   */
  sendRequest(req, payload, innerCallback) {
    const radstream = new Readable();
    radstream.push(payload);
    radstream.push(null);

    radstream.on('error', function(error) {
      logger.error(
    ...
    req.on(
      'response',
      this.createRegisterResponseHandler(this.deviceInformation, innerCallback)
    );
  ...
  startListener(innerCallback) {
    ...
    const startCoapServer = innerCallback => {
      try {
    ...
    const extractListenerInfo = (serverInfo, innerCallback) => {
      ...
      innerCallback(null, this.deviceInformation);
    };

    const startListener = innerCallback => {
      async.waterfall(
